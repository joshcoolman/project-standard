...
- **The title names the work; the body carries the argument.** A noun phrase, short enough to scan in a column — "Canvas image arrival sizing", not "Consistent sizing when an image arrives on the canvas — rules TBD". The pull is real and worth resisting: having just reasoned your way to why the work matters, the thesis is the sentence in hand, so the title becomes a claim ("Deployability is a checked property, not an intention") instead of a name. Nothing is lost by cutting it — the argument was already in the body, which is where it does its work. What is lost by keeping it is the list: titles are read a dozen at a time, side by side, and a column of theses is unscannable in a way a column of names is not. **The test: could this be a branch name?** If not, it is an argument. Drop anything after a comma, a semicolon, or a dash.
